package com.yamangulov.processor;

import com.yamangulov.SumAlertsApp;
...
import static com.yamangulov.SumAlertsApp.STATE_STORE_NAME_2;

public class SumAlertTrasformer implements Processor<String, GenericRecord, String, GenericRecord> {
    ...
    public void init(ProcessorContext<String, GenericRecord> context) {
        stateStore = context.getStateStore(STATE_STORE_NAME_2);
    ...
    public void process(Record<String, GenericRecord> record) {
        ...
        double newVal = oldVal != null ? oldVal + quantity * price : quantity * price;
    ...
    private void sendAlerts(long timestamp) {
        ...
        stateStore.range(createKey(lastProcessedWindowEnd, ""), createKey(nearestMinutesTs, ""))
                        .forEachRemaining(keyVal -> {
                            ...
                            Double sum = keyVal.value;
                            if (sum > SumAlertsApp.MAX_SUM_PER_MINUTE) {
                                ...
                                record.put("sum_of_purchases", sum);
